...
export default function ClassDetails ({ classData, toggleClear }) {
 return (
    <div className="classData-details">
     ...
     <strong>Subclass:</strong>  {classData.subclasses.map((s) => s.name)} <br />
     <strong>Proficiencies:</strong>  {classData.proficiencies.map((p) => {
        return(<ul key={p.index}>
          <li>{p.name}</li>
        </ul>)
      })}
<a data-tooltip-id="close-details-tooltip" data-tooltip-content="Click here to close details view.">
